...
{
  SetRowEnabled(WeGlidePilotDob, enabled);
...
WeGlideConfigPanel::Prepare(ContainerWindow &parent, const PixelRect &rc) noexcept
{
  ...
  AddInteger(_("WeGlide Pilot ID"),
             ...
             _T("%d ef"), _T("%d df"), 1, 99999, 1, weglide.pilot_id);

  AddDate(_("Pilot date of birth"),
      _("Date of birth to identify your WeGlide Profile. This is needed for uploading IGC files!"),
      weglide.pilot_dob);
...
WeGlideConfigPanel::Save(bool &_changed) noexcept
{
  ...
  changed |= SaveValue(WeGlideAutomaticUpload, ProfileKeys::WeGlideAutomaticUpload,
    weglide.automatic_upload);

  changed |= SaveValue(WeGlidePilotID, ProfileKeys::WeGlidePilotID,
    weglide.pilot_id);

  changed |= SaveValue(WeGlidePilotDob, ProfileKeys::WeGlidePilotDob,
    weglide.pilot_dob);
