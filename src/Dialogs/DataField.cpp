...
{
  switch (df.GetType()) {
  case DataField::Type::FILE: {
    return FilePicker(caption, (FileDataField&)df, help_text);
  }
  case DataField::Type::ROUGH_TIME: {
  ...
  case DataField::Type::DATE: {
    DataFieldDate& dfd = reinterpret_cast<DataFieldDate&>(df);
